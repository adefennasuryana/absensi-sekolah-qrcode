...
class PresensiGuruModel extends Model implements PresensiInterface
{
    ...
    public function cekAbsen(string|int $id, string|Time $date)
    {
        ...
        return $result ? $result[$this->primaryKey] : false;
    }

    public function absenMasuk(string $id, $date, $time)
    {
        $existing = $this->getPresensiByIdGuruTanggal($id, $date);

        if ($existing) {
            // Update jika sudah ada presensi hari ini
    ...
    public function getPresensiByKehadiran(string $idKehadiran, $tanggal)
    {
        ...
        if ($idKehadiran == '4') {
            ...
            // Filter jika bukan hadir/sakit/izin
            $filteredResult = array_filter($result, function ($value) {
                return !in_array($value['id_kehadiran'], ['1', '2', '3']);
            });
